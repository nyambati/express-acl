...
(function() {
  ...
  var authorize = function(req, res, next) {
    ...
    var role, resource, methods, action, group, length, message, policy, currResource;
    ...
    if (length >= 1 && resource !== '*') {
      ...
      if (methods && _.isString(methods)) {
        action = policy.action;
        return utils.whenGlobAndMethodString(res, next, action, methods);
      }
      ...
      if (_.isArray(methods)) {
        action = policy.action;
        return utils.isArrayMethod(res, next, action, method, methods);
      }
