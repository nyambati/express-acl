...
(function() {
  ...
  var config = function(options) {
    var path, rules, encoding, filename;

    /**
     * Get the filename
     */

    filename = options && options.filename ? options.filename : 'config.json';
    ...
    rules = helper.readJson(path, encoding);
  ...
  var authorize = function(req, res, next) {
    ...
    var role, resource, methods, index, action, group, length, boolean, policy;
    ...
    if (length === 1 && resource === '*' && policy.action === 'allow') {
      /*
    ...
    if (length === 1 && resource === '*' && policy.action === 'deny') {
      /*
       * Allow traffic to all  resources
       * 1. Check for methods
       * 2. If its a string and a glob '*'
