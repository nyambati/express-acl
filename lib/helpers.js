(function() {
  ...
  */
  var checkProperties = function(error, config) {
    var permissions;

    config.forEach(function(el) {
      if ((typeof el.group === 'string') && (Array.isArray(el.permissions))) {
      ...
      return log.error('REQUIRED:', error);
    });
    return config;
  };
    ...
    */
    validate: function(config) {
      if (!Array.isArray(config)) {
        ...
        return checkProperties(this.error.required, config);
      }
    },
