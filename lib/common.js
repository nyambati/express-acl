 ...
 */
const urlToArray = url => url.replace(/^\/+|\/+$/gm, '').split('/');
...
const mapPolicyToGroup = policies => {
  ...
  policies.forEach(policy => {
    ...
    policy.permissions.forEach(permission => {
      ...
      assertIsGlob(permission.methods, 'Methods');
...
const matchUrlToResource = (route, resource) => {
  // if the resource is a glob return true
  ...
  // if the length of the resource array is larger that route Array
  //  and the resource does not contain  a glob

  if (routeArray.length < resourceArray.length && !resourceArray.includes('*'))
    return false;
...
const isAllowed = (method, permission, customMessage, response) => {
